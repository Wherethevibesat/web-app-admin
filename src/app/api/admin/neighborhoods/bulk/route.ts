...
export async function POST(request: Request) {
  ...
  try {
    let result;
    if (action === "delete") {
      result = await bulkDeleteNeighborhoods(ids);
    } else if (action === "activate") {
      result = await bulkPatchNeighborhoods(ids, { is_active: true });
    } else if (action === "deactivate") {
      result = await bulkPatchNeighborhoods(ids, { is_active: false });
    } else {
    ...
    if (result.succeeded.length) {
      await logAdminAction({
        ...
        action: `neighborhood.bulk_${action}`,
