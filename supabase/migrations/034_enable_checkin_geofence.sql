-- Enforce location on check-in (geofence)
-- Run after 033_leaderboard_window.sql
--
-- Recreates check_in_venue with the geofence turned ON:
--   * requires device coordinates when the venue has coordinates
--   * rejects check-ins farther than the radius (+ a capped GPS-accuracy slack)
--   * venues without saved coordinates cannot be validated yet, so they are
--     allowed but flagged (owners should set the venue location)
--
-- Adds a p_accuracy argument, so the old 4-arg signature is dropped first.

DROP FUNCTION IF EXISTS public.check_in_venue(TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
  ...
  IF v_geofence_m > 0 AND v_venue.latitude IS NOT NULL AND v_venue.longitude IS NOT NULL THEN
    ...
    -- Give the benefit of the doubt up to the device's reported accuracy, capped.
...
GRANT EXECUTE ON FUNCTION public.check_in_venue(TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION) TO authenticated;
